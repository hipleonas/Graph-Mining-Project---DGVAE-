...
class DGVAE(nn.Module):
    ...
    def __init__(
        ...
        self.collabfilter_network = MacridVAE(
        ...
        self.text_network = MacridVAE(
            item_adj=self.words_adj,
    ...
    def forward(self, input_rating, input_text, need_prob_k = False):
        cf_z_list, cf_mu_list, cf_logvar_list, cf_cates_k_list, cf_items, cf_prototypes = self.collabfilter_network.encode(input_rating)

        text_z_list, text_mu_list, text_logvar_list, text_cates_k_list, text_items, text_prototypes = self.text_network.encode(input_text)

        assert len(text_z_list) == len(cf_z_list), "The number of layers in the text and cf networks must be the same"

        cf_z_tensor = torch.stack(cf_z_list, dim =0).permute(1,0,2)
        text_z_tensor = torch.stack(text_z_list, dim = 0).permute(1,0,2)

        text_z_tensor_given_cf = self.attn_layer(q_mat=cf_z_tensor,
                                                      k_mat=text_z_tensor,
                                                      v_mat=text_z_tensor)
        text_z_list_given_cf = self.split(text_z_tensor_given_cf)
        cf_z_tensor_given_text = self.attn_layer(q_mat=text_z_tensor,
                                                      k_mat=cf_z_tensor,
                                                      v_mat=cf_z_tensor)
        cf_z_list_given_text = self.split(cf_z_tensor_given_text)

        regularization = self.mim(cf_z_list_given_text, text_z_list_given_cf)
        z_list = [cf_z_list_given_text[i] + text_z_list_given_cf[i] for i in range(self.num_prototypes)]
        cf_probs, cf_prob_k_list = \
            self.collabfilter_network.decode(z_list, cf_cates_k_list, cf_items, need_prob_k)
        ...
        if need_prob_k:
            return cf_logits, text_logits, regularization, cf_logvar_list, text_logvar_list, \
                   cf_prob_k_list, text_prob_k_list
        return cf_logits, text_logits, regularization, cf_logvar_list, text_logvar_list
    def attn_layer(self, query, key, value):
        p1 = (query.bmm(key.transpose(1,2)))
        p1 = p1.tanh()
        p2 = -(query.unsqueeze(dim=2) - key.unsqueeze(dim=1)).abs().sum(dim=-1)
        p2 = (p2 - p2.mean(dim=-1, keepdim=True)).sigmoid()
        return (p1 * p2).bmm(value) #Compute Attention Score layer


    def mim(self, list_a: List[Tensor], list_b: List[Tensor]):
        assert len(list_a) == len(list_b), "The number of layers in the two lists must be the same"
        assert len(list_a) == self.num_prototypes, "The number of prototypes must match the length of the lists"

        mi_regularization = None

        for p_idx in range(self.num_prototypes):
            x_pos = list_a[p_idx]
            y_pos = list_b[p_idx]   
            f_vals = [(x_pos * list_b[k]).sum(dim = -1) for k in range(self.num_prototypes)]

            soft_plus = [F.softplus(value).unsqueeze(dim=-1) for value in f_vals]

            en = torch.cat(soft_plus, dim=-1)
            ep = -F.softplus(-(x_pos * y_pos).sum(dim=-1))
            Loss_I = (en - ep).mean()

            mi_regularization = Loss_I if mi_regularization is None else Loss_I + mi_regularization
        return mi_regularization / self.num_prototypes
    @staticmethod
    ...
    def compute_loss(self, input_rating, input_text):
        ...
        cf_kl_loss = self.collabfilter_network.calculate_kl_loss(cf_logvar_list)
        cf_ce_loss = self.collabfilter_network.calculate_ce_loss(input_rating, cf_logits)
        if self.collabfilter_network.reg_weights[0] != 0 or self.collabfilter_network.reg_weights[1] != 0:
            cf_loss = cf_ce_loss + cf_kl_loss * rating_anneal + self.collabfilter_network.reg_loss()
        else:
        ...
        total_loss = cf_loss + self.lambda_text * text_loss
        if self.lambda_reg > 0:
            total_loss = total_loss + self.lambda_reg * reg_term
        return total_loss
    ...
    def predict(self, input_rating, input_text):
        rating_pred, text_pred, _, _, _ = self.forward(input_rating, input_text)
        rating_pred[input_rating.nonzero(as_tuple=True)] = -np.Inf
...
class MacridVAE(nn.Module):
    ...
    def encode(self, input_rating):
        prototypes = F.normalize(self.prototype_embeddings.weight, dim=1)
        items = F.normalize(self.item_embeddings.weight, dim=1)
