...
def get_string_config(args):
    ...
    #     return f'{args.dataset}_' + '_'.join([str(clean(args.__dict__[attr])) for attr in selected_attr])
    ...
    if args.file_name is not None:
        selected_attr = [attr for attr in args.file_name.split(',')]
        return f'{args.dataset}_' + '_'.join([clean(args.__dict__[attr]) for attr in selected_attr])

    # Fallback: dùng clean() cho toàn bộ phần còn lại
    return '_'.join([
        clean(value) for attr, value in args.__dict__.items()
        if attr not in excluded_attributes and value is not None
    ])
    # '''
    # New update code
    # '''
    ...
    #     selected_attr = [attr for attr in args.file_name.split(',')]
    #     try:
    #         return f'{args.dataset}_' + '_'.join([
    #             str(args.__dict__[attr]) for attr in selected_attr if args.__dict__[attr] is not None
    #         ])
    #     except KeyError:
    #         pass
    # return f'{args.dataset}_' + '_'.join([
    #     str(value) for attr, value in args.__dict__.items()
    #     if attr not in excluded_attributes and value is not None
    # ])
...
def train(model, train_loader, test_loader, optimizer, device):
    ...
    print(f"[DEBUG] Saving model to: {model_file}")
